
texts = {
    'ru': {

    'welcome': 
            {
        
        'hello_msg': 'Вас приветсвует центр современных профессий <b>PROWEB!</b>🤗',

        'greeting': f'''
Данный бот разработан специально для студентов центра <b>PROWEB.</b>

Возможности бота:

▪️ <b>Отправить текстовое сообщение</b> - задать любой вопрос, связанный с вашим обучением, и получить обратную связь
в ближайщее время.

▪️ <b>Тех. поддержка</b> - связаться с технической поддержкой и решить вопросы, связанные с компьютерной техникой. Также можно получить
консультацию и помощь в сборке личного компьютера.

▪️ <b>Коворкинг</b> - связаться с коворкинг админстратором и забронировать место на посещение.

▪️ <b>Конкурсы </b> - принять участие в ежемесячных конкурсах, где можно выиграть ценные призы.

▪️ <b>Посетить сайт</b> - переход на страницу центра PROWEB, где вы найдете всю подробную информацию о центре и о доступных курсах.

▪️ <b>Базовый курс</b> - записаться на бесплатный базовый курс компьютерной грамотности.

▪️ <b>Оставить отзыв</b> - возможность поделиться приятными впечетлениями об обучении в центре PROWEB. Если столкнулись с трудностями,
можно оставить жалобу или пожелание. Мы благодарны каждому отзыву.

▪️ <b>Правила обучения</b> - подробная информация о правилах обучения в центре PROWEB.

▪️ <b>На главную</b> - возможность вернуться на главную страницу бота и увидеть все его возможности.

▪️ <b>O'zbek tili</b> - переключение языка с русского на узбекский.

▪️ <b>Поделиться контактом</b> - если ваш вопрос имеет личный характер, оставьте свои данные и администрация центра
свяжется с вами лично в ближайщее время.
'''},

    'konkursi': {
        'photo': '',
        'text': f'''

Ежемесячно для студентов центра <b>PROWEB</b> проводятся конкурсы с ценными призами!🎉

<a href="https://proweb.uz/img/bonusMob.48c11c86.webp">&#8205;</a>

Для участия в конкурсах важно оставаться подписанным на нашего бота.

Подробные объявления с условиями конкурсов будут приходить личным сообщением всем подписчикам. Для того, чтобы оставаться
подписанным, достаточно просто не удалять и не блокировать бота.

Также мы проводим дополнительные  конкурсы длянаших подчисчиков в Instagram, Youtube, Telegram, Facebook - подписывайтесь
и участвуйте сразу в нескольких конкурсах.

Всех удачи, шанс выиграть есть у каждого студента!🍀
''',
    }
    ,


    'base_course': f'''
    Вы ссовершенно не разбираетесь в компьютере, но при этом хотите обучаться в <b>PROWEB?</b>
С нашим базовым курсом это возможно. Для этого вам просто необходимо нажать на кнопку "<b>Записаться на базовый курс</b>"
и наш преподаватель вас подробно обо всём проконсультрует.
<a href="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSbOUYyx-etIXXSqfPAqohjX0LuKCUWjPWj9A&s">&#8205;</a>
    ''',


    'wishes': f'''
Вам нравится обучение в центре <b>PROWEB</b> и вам не трудно поделится об этом в наших социальных сетях? Тогда переходите по ссылке "<b>Отзывы</b>"
и опишите свои впечатления, мы будем очень признательны.

У вас возникли трудности во время обучения или вам что-то не понравилось, а может есть предложения и пожелания? Тогда переходите пл ссылке "<b>Жалобы и пожелания</b>",
распишите подробнее и мы сделаем все возможное, чтобы улучшить наш учебный центр.
''',
    
    'complaints': f'''
С какими сложностями вы столкнулись или какие у вас есть предложения по улучшению центра <b>PROWEB</b>?

Пожалуйста, распишете подробнее и отправьте сообщением.

Вы также можете "<b>Поделиться контактом</b>", наша администрация свяжется с вами лично и выслушает все ваши замечания.
''',


    'rules': f'''
<b>Правила обучния в учебном центре PROWEB</b>

1. В одном учебном месяце - 8 занятий. Студентам, оплатившим месяц обучения гарантируется 8 проведенных уроков по утвержденному графику. 

2. Если урок отменяется в связи с праздником или по уважительной причение со стороны преподавателя, урок ни в коем случае не сгорает, а просто переносится. Пока все 8 официальных уроков не будут проведены, учебный месяц не закончится. 

3. Если студент пропускает запланированный урок, данный урок засчитывается, так как студент состоит в Telegram группе, в которую скидывается каждый пройденный урок в видео формате.

4. Если студент не может посетить урок, он должен оповестить об этом преподователя и администрацию.

5. Если студент пропустил урок, то ему рекомендуется посетить коворкинг, в котором находятся преподаватели всех направлений. Преподаватели коворкинга могу помочь разобрать пропущенную тему и помочь в сложных учебных моментах.

6. В период обучения студент может посещять коворкинг каждый день, заранее регистрируясь в доступное время. Для регистрации необходимо связаться с преподавателем коворкинга и забронировать место.

7. Перед стартом курса, или уже во время обучения, студентам с плохими знаниями компьютера рекомендуется записаться на беспплатный базовый курс. За месяц студенты изучат основы компьютерной грамотности. Обучние проходит параллельно с основным курсом. Для записи нужно написать в Telegram.

8. Каждый студент учебного центра PROWEB должен вносить своевременную оплату за каждый месяц обучения в начале учебного месяца, во избежание исключения из группы. Если студент имеет финансовые трудности, то администрация может предоставить недельную отсрочку в выплате, либо студент может вносить частичную оплату в период текущего месяца обучения. 

9. Если студент не вносит своевремненную оплату и не посещяет более трех уроков без уважительной причины, то данный студент автоматически исключается из группы. Для восстановления в группу студент должэен погаситьь задолженность, только после этого он будт восстановлен в свою группу, если та не ушла вперёд. Либо администрация подберет группу, наиболее подходящую данному студенту.

10. Если студент хочет прикратить или приостановить обучение, нужно обязательно оповестить администрацию и объяснить причину, чтобы будущие уроки, которые студент не будет посещять, не засчитывались. Администрация своевременно приостановит обучение и удалит студента из Telegram группы.

11. Студент, приостановивший обучение, может восстановить своё обучние ровно с того месяца и урока, на котором приостановил. Администрация не гарантирует, что восстановится получится именно к своему преподавателю.

12. Если студент хочет сменить формат обучения, преподаватели, или изменить врмея и дни обучения, он может обратиться в администрацию центра, которая обязательно ему в этом поможет.

13. Студент, имеющий жалобу или предложение, может обратиться в администрацию учебного центра или к менеджеру обратной связи. Все жалобы и предложения будут обязательно расммотрены и будут приняты соответсвующие меры. 

<b>Контакты:</b>

<b>Администрация</b>
Telegram: t.me/proweb_admin
Tel: +97 744 66 32

<b>Обратная связь</b>
Telegram: t.me/proweb_lost_back
Tel: +88 198 66 32

<b>Коворкинг</b> 
Telegram: t.me/proweb_admin
Tel: +97 155 66 32

<b>Базовый курс</b> 
Telegram: t.me/proweb_basics
''',

    'confirm_admin': f'<b>PROWEB</b> хочет назначить вас администратором, вы подтверждаете?',


    'reply_btns': 
    
    {
        'main': [
        
        {'text': 'На главную ↩️'},
        {'text': "O'zbek tili 🇺🇿"},
        {'text': "Поделится контактом", 'request_contact': True},

    ],
    
        'mailing': [
        {'text': 'Рассылка в личные чаты пользователей'},
        {'text': 'Рассылка в группы студентов'},

    ],
    },


    'inline_btns': 
    
    {'main': [
        
        {'text': 'Тех. поддержка', 'url': 't.me/itsmylifestyle'},
        {'text': "Коворкинг", 'url': 't.me/proweb_coworking'},
        {'text': "Конкурсы🎉", "callback_data":'konkursi'},
        {'text': "Посетить сайт", 'url': 'proweb.uz'},
        {'text': "Базовый курс",'callback_data': 'base_course'},
        {'text': "Оставить отзыв",'callback_data': 'comment'},
        {'text': "Правила обучния", 'callback_data': 'rules'},

    ],
    
        'base_course': [{'text': 'Записаться на Базовый курс', 'url': 't.me/proweb_basics', 'callback_data': None}],

        'wishes': [
            {'text': 'Отзывы 😍', 'url': 'https://proweb.uz/reviews/', 'callback_data': None},
            {'text': 'Жалобы и пожелания 😔', 'url': None, 'callback_data': 'complaint'},
        ],


        'confirm_admin': [
            {'text': 'Да, подтвеждаю ✅', 'callback_data': 'confirm'},
            ],
    },

    

    },




    'uz': 
    {
        'welcome': {
        
        'hello_msg': '<b>PROWEB</b> zamonaviy kasblar markaziga xush kelibsiz!🤗',
        
        'greeting': f'''
Bu bot maxsus <b>PROWEB</b> markazi talabalari uchun yaratilgan.

Bot imkoniyatlari:

▪️ <b>Matnli xabar yuboring</b> - mashg'ulotingiz bilan bog'liq har qanday savolni bering 
va iloji boricha tezroq qayta aloqa oling.

▪️ <b> Texnik yordam</b> - texnik yordam bilan bog'laning va kompyuter texnologiyalari bilan bog'loq muammolarni hal qiling.
Shuningdek, siz shaxsiy kompyuterni yig'ish bo'yicha maslahat va yordam oling.

▪️ <b>Kovorking</b> - kovorking administratoriga murojaat qiling va tashrif bbuyurishga joyni bron qiling.

▪️ <b>Tanlovlar</b> - har oylik tanlovlarda qatnashib, qimmatbaho sovrinlarni yutib oling.

▪️ <b>Saytga tashrif buyuring</b> - PROWEB markazining sahifasiga o'ting, u erda siz markaz va mavjud kurslar haqidagi barcha batafsil ma'lumotlarni topasiz.

▪️ <b>Kompyuter asoslari</b> - kompyuter asoslari kursiga yozilish.

▪️ <b>Sharh qoldiring</b> - PROWEB markazida mashg'ulotlardan yoqimli taassurotlar bilan bo'lishish imkoniyati. Agar
qiyinchiliklarga duch kelsangiz, shikoyat yoki so'rov qoldirishingiz mumkin. Biz har bir fikr uchun minnatdormiz.

▪️ <b>O'qish qoidalari</b> - PROWEB markazida o'qish qoidalari haqida batafsil ma'lumot.

▪️ <b>Bosh sahifaga</b> - botning asosiy sahifasiga qaytish va imkoniyatlari haqida batafsil ma'lumot olish.

▪️ <b>O'zbek tili</b> - tilni o'zbek tilidan rus tiliga o'tkazish.

▪️ <b>Kontakt bilan ulashing</b> - agar sizning savolingiz shaxsiy xarakterga ega bo'lsa, ozma'lumotlaringizni qoldiring va
markaz ma'muriyati imkon qadar tezroq siz bilan shaxsan bog'lanadi. 
'''},

    'konkursi': {
        'photo': open('common/media/photo.webp', 'rb'),
        'text': f'''<a href="https://proweb.uz/img/bonusMob.48c11c86.webp">&#160;</a>
Qimmatbaho sovrinli tanlovlar har oy <b>PROWEB</b>
markazi o'quvchilari uchun o'tkaziladi!🎉

Tanlovlarda qatnashish uchun botimizga obuna bo'lishingiz kerak.

Tanlov shartlari ko'rsatilgan batafsil xabarlar barcha obunachilarga shaxsiy xabar orqali yuboriladi. Obunani
saqlab qolish uchun botni to'xtatmaslik yoki bloklamaligingiz kifoya.

Shuningdek, biz Instagram, Youtube, Telegram, Facebookdagi obunachilarimiz uchun qo'shimcha tanlovlar o'tkazamiz - obuna
bo'ling va bir vaqtning o'zida bir nechta tanlovlarda ishtirok eting.

Hammaga omad, har bir talabada g'alaba qozonish imkoniyati bor!🍀
'''},


    'base_course': f'''
Sizda umuman kompyuter bilimlari yo'q, lekin <b>PROWEB</b>da o'qishni xohlaysizmi? Bu bizning kompyuter asoslari kursimiz bilan mumkin.
Buning uchun "<b>Kompyuter asoslari kursiga yozilish</b> tugmasini bosish kifoya va o'qituvchimiz sizga hamma narsa haqida batafsil maslahat beradi."
<a href="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSbOUYyx-etIXXSqfPAqohjX0LuKCUWjPWj9A&s">&#8205;</a>
''',


    
    'wishes': f'''
Sizga <b>PROWEB</b> markazida oq'ish yoqadimi va bu haqda ijtimoiy tarmoqlarimizda bo'lishish siz uchun qiyin emasmi? Bo'lmasam "<b>Sharhlar</b>
" havolasiga o'ting va taassurotlaringizni ta'riflang, biz sizga juda minnatdor bo'lamiz.

Agar siz mashg'ulot paytida qiyinchiliklarga duch kelsangiz yoki sizga biror narsa yoqmagan bo'lsa, ehtimol sizda taklif va istaklar bor bo'lsa?
Bo'lmasam "<b>Shikoyat va istaklar</b> havolasiga o'ting, hamma narsani batafsil tavsiflab bering va biz o'quv markazimizni yaxshilash uchun"
qolimizdan kelganini qilamiz.
''',


    'complaints': f'''
Qanday qiyinchilikarga duch keldingiz yoki <b>PROWEB</b> markazini takomillashtirish bo'yicha qanday takliflaringiz bor?

Iltimos, batafsilroq tavsiflab bering va xabar qilib
yuboring.

Siz shuningdek "<b>Kontakt bilan ulashish</b>" ingiz mumkin, bizning ma'muriyat siz bilan shaxsan bog'lanadi va sharhlaringizni linglaydi.
''',


    'rules': f'''
<b>PROWEB oq'uv markazida o'qish qoidalari</b>

1. Bir o'quv oyida - 8 ta dars. Bir oy o'qish uchun pul to'lagan o'quchilarga tasdiqlangan jadval bo'yicha 8 ta dars o'tkazilishi kafolatlanadi.

2. Agar dars bayram yoki o'qituvchi tomonidan uzrli sabablarga ko'ra bekor qilinsa, dars hech qanday holatda kuymaydi, balki keyinga qoldiriladi. Bir akademik oyda 8ta dars bor. Barcha 8ta rasmiy darslar bajarilmaguncha, o'quv oyi tugamaydi.

3. Agar o'quvchi rejalashtirilgan darsni qoldirsa, bu dars hisobga olinadi, chunki har bir dars video formatida Telegramdagi guruhga jo'natiladi.

4. Agar o'quvchi darsga qatnasha olmasa, u bu haqida o'qituvchi va ma'muriyatga xabar berishi shart.

5. Agar o'quvchi u holda tomonlama o'qituvchilar joylashgan kovorking xanaga tashrif buyurish tavsiya etiladi, u yerda har bir yo'nalishdan o'qituvchilar bo'ladi.
Kovorking o'qituvchilari o'kazib yuborilgan mavzuni va o'qishdagi qiyinchiliklari bilan yordam berishlari shart.

6. O'qish mobaynida o'quvchi har kuni kovorking xonasiga mavjud vaqtga oldindan ro'yxatdan o'tib, tashrif buyurishi mumkin. Ro'yxatdan o'tish  uchun kovorking o'qituvchisiga murojaat qilishi va joy bron qilishi kerak.

7. Kursni boshlashdan oldin yoki mashg'ulot paytida kompyuter bilimi kam bo'lgan o'quvchilarga bepul mopyuter asoslari kursiga yozilish tavsiya etiladi. Bir oy ichida o'quvchilar kompyuter savodxonligi asoslarini o'rganadilar. O'qish asosiy kurga parallel ravishda o'tkaziladi. Ro'yxatdan o'tish uchun siz Telegramda kompyuter asoslari kursi o'qituvchisiga murojaat qilishingiz kerak.

8. PROWEB o'quv markazining har bir o'quvchisi, guruhdan chetlatilishining oldini olish uchun, o'quv oyining boshida o'qish oyi uchun yo'lovni o'z vaqtida to'lashi kerak. Agar o'quvchi moliyaviy qiyinchiliklarga duch kelsa, ma'muriyat har haftalik to'lashga imtiyozli davrnli taqdim etishi yoki o'quvchi joriy o'qish davriga qisman to'lashi mumkin.

9. Agar o'quvchi o'z vaqtida to'lamasa va uzrli sababsiz uchdan ortiq darsda qatnashmasa, bu o'quvchi avtomatik ravishd guruhdan chiqariladi.
Guruhga qayta tiklanish uchun o'quvchi qarzini to'lashi kerak, shundan keyingina u o'z guruhiga qaytariladi, agar u guruh oldinga o'tib ketgan bo'lmasa. Yoki ma'muriyat ushbu o'quvchiga eng mos keladigan guruhni tanlab beradi.

10. Agar o'quvchi o'qishni to'xtatishni xohlasa, u ma'muriyatni xabardor qilishni va sababini tushuntirishi kerak, shunda o'quvchi kelmaydigan darslar hisobga olinmaydi. Ma'muriyat mashg'ulotlarni o'z vaqtida to'xtatib qo'yadi, o'quvchini Telegram guruhidan chiqaradi va mashg'ulotlarni tox'tatadi.

11. O'qishni to'xtatgan o'quvchi, o'qishni to'xtatgan oyi va darsidan boshlab davom ettirishi mumkin. Ma'muriyat uni o'z o'qituvchisiga qaytarilishini kafolatlamaydi.

12. Agar o'quvchi o'quvchi o'qish formatini, o'qituvchini yoki mashg'ulot vqti va kunlari o;zgartimoqchi bo'lsa, u markaz ma'muriyati bilan bog'lanishi mumkin, ular albatta bu istaklar bilar yordam berishadi.

13. Shikotat yoki taklifga ega bo'lgan o'quvchi qayta aloqa menejeri yoki o'quv markazi ma'muriyatiga murojaat qilishi mumkin. Barcha shikoyat va takliflar albatta ko'rib chiqiladi va tegishli choralar ko'riladi.

<b>Kontaktlar:</b>

<b>Markaz ma'muriyati</b>
Telegram: t.me/proweb_admin
Tel: +97 744 66 32

<b>Qayta aloqa menejeri</b>
Telegram: t.me/proweb_lost_back
Tel: +88 198 66 32

<b>Kovorking</b> 
Telegram: t.me/proweb_admin
Tel: +97 155 66 32

<b>Kompyuter asoslari kursi</b> 
Telegram: t.me/proweb_basics
''',

    'confirm_admin': f'<b>PROWEB</b> sizni administrator sifatida tayinlamoqchi, tasdiqlaysizmi?',

    'reply_btns': 
    
    {
        'main': [
        
        {'text': 'Bosh sahifaga ↩️'},
        {'text': "Русский язык 🇷🇺"},
        {'text': "Kontakt bilan ulashing", 'request_contact': True},

    ],

    'mailing': [
        {'text': 'Foydalanuvchilarning shaxsiy suhbatlariga yuborish'},
        {'text': 'Talabalar guruhlariga yuborish'},

    ],
    
    
    },


    'inline_btns': 
    
    {
        'main': [
        
        {'text': 'Texnik yoram', 'url': 't.me/itsmylifestyle'},
        {'text': "Kovorking", 'url': 't.me/proweb_coworking'},
        {'text': "Tanlovlar🎉", "callback_data":'konkursi'},
        {'text': "Saytga tashrif buyurish", 'url': 'proweb.uz'},
        {'text': "Kompyuter asoslari", 'callback_data': 'base_course'},
        {'text': "Sharh qoldirish", 'callback_data': 'comment'},
        {'text': "O'qish qoidalari", 'callback_data': 'rules'},

    ],
    
    'base_course': [{'text': 'Kompyuter asoslari kursiga yozilish', 'url': 't.me/proweb_basics'}],
    

    'wishes': [
            {'text': 'Sharhlar 😍', 'url': 'https://proweb.uz/uz/reviews/'},
            {'text': 'Shikoyat va istaklar 😔', 'callback_data': 'complaint'},
            ],
    
    'confirm_admin': [
            {'text': 'Ha, tasdiqlayman ✅', 'callback_data': 'confirm'},

    ],

    },
    

    },

}


